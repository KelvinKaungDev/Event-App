...
struct OrganizingEventListView: View {
    ...
    @State var organisingEventList: [Events] = []
    
    var body: some View {
        List(organisingEventList, id: \.id) { event in
                ...
                viewModel.fetchUserPopulateEventList(userId: userId) { result in
                    DispatchQueue.main.async {
                        ...
                        case .success(let populateEventList):
                            self.organisingEventList = populateEventList.organisingEventList ?? []
